...
\begin{frame}
  ...
  \begin{theorem}[3.4: Strictly Increasing Risk Aversion]
    If risk aversion strictly increases next period from state $i$ and $R$ is bounded below by $m > 0$, then
    \begin{equation*}
...
\begin{frame}
  ...
  \begin{itemize}
    \item Single, clear and simple mechanism, does not rely on growth rates.
    \item Robust theoretical result.
  \end{itemize}

  \structure{Why I want to know more:}
  \begin{itemize}
    \item Do we need limiting MPCs to be zero? What \alert{specific empirical fact} are we trying to explain?
    \item Is this a story about \alert{risk} or \alert{inter-temporal substitution}?
    \item Overall, we are now \alert{left with lots of results} on MPCs and tails, with fruit salad of \alert{ad hoc growth conditions}. 
    \item Does risk aversion dynamics pass the \underline{\alert{"Kath Day-Night Test"}}?
  \end{itemize}
\end{frame}
